...
def select_contribute(request):
    ...
    try:
        ...
        customer_list = Customer.objects.values('id', 'name').annotate(sum=Sum('customerorders__ordersdetail__sum')).order_by('-sum')
        #按条件查询
        #客户名称
...
def select_serve(request):
    ...
    serve = CustomerServe.objects.values('serveType').\
                    annotate(type=Case(When(serveType=6, then=Value("咨询")),
                                       ...
                                       When(serveType=8, then=Value("投诉"))),
                             amount=Count('serveType')).\
                        order_by('serveType')
...
def select_loss(request):
    """查询客户流失数据"""
    loss = CustomerLoss.objects.values().filter(state=1).order_by('-id').all()

    return JsonResponse(list(loss), safe=False)
